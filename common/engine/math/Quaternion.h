...
public:
	...
	float distanceFrom(const Quaternion&) const;
	float distanceFromSq(const Quaternion&) const;
...
inline void Quaternion::setXYZ(float x, float y, float z)
{
	X = x;
	Y = y;
	Z = z;
...
inline float Quaternion::distanceFrom(const Quaternion& v) const
{
	return Vector4(X - v.X, Y - v.Y, Z - v.Z, W - v.W).length(); 
}

inline float Quaternion::distanceFromSq(const Quaternion& v) const
{
